import { useState } from 'react';
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { BarChart3, Loader2, ArrowLeft, Sparkles } from 'lucide-react';
import { SignedIn, SignedOut, RedirectToSignIn } from '@clerk/clerk-react';
import Navbar from "@/components/Navbar";
import Footer from "@/components/Footer";
import AnimatedBackground from "@/components/AnimatedBackground";
import LimitReachedModal from "@/components/LimitReachedModal";
import FormattedOutput from "@/components/FormattedOutput";

const JudgeAnalytics = () => {
  const [judgeName, setJudgeName] = useState('');
  const [court, setCourt] = useState('');
  const [isProcessing, setIsProcessing] = useState(false);
  const [analytics, setAnalytics] = useState<any>(null);
  const [showLimitModal, setShowLimitModal] = useState(false);

  const handleAnalyze = async () => {
    setIsProcessing(true);
    setAnalytics(null);
    try {
      const response = await fetch('http://localhost:8000/api/lawyer/judge-analytics', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ judge_name: judgeName, court }),
      });
      if (response.status === 429) { setShowLimitModal(true); return; }
      if (response.ok) setAnalytics(await response.json());
      else alert('Failed to generate analytics. Please try again.');
    } catch { alert('An error occurred. Please try again.'); }
    finally { setIsProcessing(false); }
  };

  const resultText = analytics?.analytics
    ? (typeof analytics.analytics === 'string'
      ? analytics.analytics
      : JSON.stringify(analytics.analytics, null, 2))
    : '';

  return (
    <>
      <SignedOut><RedirectToSignIn /></SignedOut>
      <SignedIn>
        <div className="min-h-screen relative page-transition">
          <AnimatedBackground />
          <Navbar />
          <div className="pt-24 pb-20 relative z-10">
            <div className="container mx-auto px-4 max-w-6xl">

              <div className="mb-10">
                <button
                  onClick={() => window.location.href = '/lawyer-dashboard'}
                  className="inline-flex items-center gap-2 text-slate-400 hover:text-white text-sm font-medium mb-6 transition-colors duration-200"
                >
                  <ArrowLeft className="h-4 w-4" /> Back to Dashboard
                </button>
                <div className="flex items-center gap-3 mb-3">
                  <span className="badge-purple"><BarChart3 className="h-3.5 w-3.5" /> Lawyer Tool</span>
                </div>
                <h1 className="page-title mb-2">Judge Analytics</h1>
                <p className="page-subtitle">Anonymised judicial pattern insights for research and hearing preparation.</p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">

                <div className="tool-card p-6 flex flex-col gap-5">
                  <div>
                    <h2 className="text-base font-semibold text-slate-200 mb-1">Search Parameters</h2>
                    <p className="text-xs text-slate-500">Both fields are optional — leave blank for general insights.</p>
                  </div>
                  <div className="flex flex-col gap-4">
                    <div>
                      <label className="text-xs text-slate-400 mb-1.5 block">Judge Name (Optional)</label>
                      <Input
                        placeholder="e.g., Justice D.Y. Chandrachud"
                        value={judgeName}
                        onChange={(e) => setJudgeName(e.target.value)}
                        className="bg-slate-900/60 border-slate-700 text-slate-200 placeholder:text-slate-600"
                      />
                    </div>
                    <div>
                      <label className="text-xs text-slate-400 mb-1.5 block">Court (Optional)</label>
                      <Input
                        placeholder="e.g., Supreme Court of India"
                        value={court}
                        onChange={(e) => setCourt(e.target.value)}
                        className="bg-slate-900/60 border-slate-700 text-slate-200 placeholder:text-slate-600"
                      />
                    </div>
                  </div>
                  <Button
                    onClick={handleAnalyze}
                    disabled={isProcessing}
                    className="btn-primary w-full h-11 text-sm font-semibold"
                  >
                    {isProcessing
                      ? <><Loader2 className="mr-2 h-4 w-4 animate-spin" />Generating Analytics...</>
                      : <><BarChart3 className="mr-2 h-4 w-4" />Generate Judge Analytics</>}
                  </Button>
                </div>

                <div className="tool-card p-6 flex flex-col gap-4">
                  <div>
                    <h2 className="text-base font-semibold text-slate-200 mb-0.5">Analytics Results</h2>
                    <p className="text-xs text-slate-500">Judicial patterns and preparation insights</p>
                  </div>
                  <div className="flex-1 min-h-[420px]">
                    {!analytics && !isProcessing && (
                      <div className="h-full flex flex-col items-center justify-center text-slate-600 gap-3">
                        <div className="w-14 h-14 rounded-2xl bg-slate-800/80 flex items-center justify-center">
                          <BarChart3 className="h-7 w-7 opacity-40" />
                        </div>
                        <p className="text-sm">Analytics will appear here</p>
                      </div>
                    )}
                    {isProcessing && (
                      <div className="h-full flex flex-col items-center justify-center gap-4">
                        <div className="relative">
                          <div className="w-14 h-14 rounded-full border-2 border-blue-500/20 border-t-blue-500 animate-spin" />
                          <Sparkles className="absolute inset-0 m-auto h-5 w-5 text-blue-400" />
                        </div>
                        <p className="text-sm text-slate-300 font-medium">Generating analytics...</p>
                      </div>
                    )}
                    {analytics && (
                      <div className="result-box max-h-[480px] overflow-y-auto scrollbar-hide p-4">
                        <FormattedOutput content={resultText} />
                      </div>
                    )}
                  </div>
                </div>

              </div>

              <div className="disclaimer-box mt-8">
                <strong>Research Notice:</strong> Analytics are based on publicly available information and AI-generated insights. Not a substitute for professional legal judgment.
              </div>

            </div>
          </div>
          <Footer />
          <LimitReachedModal isOpen={showLimitModal} onClose={() => setShowLimitModal(false)} type="summarization" />
        </div>
      </SignedIn>
    </>
  );
};

export default JudgeAnalytics;
